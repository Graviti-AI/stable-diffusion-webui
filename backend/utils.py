...
if TYPE_CHECKING:
    from modules.model_info import ModelInfoProtocal
...
def load_torch_file(model_info: Union["ModelInfoProtocal", str], safe_load=False, device=None):
    if isinstance(model_info, str):
        filename = model_info
        is_safetensors = filename.lower().endswith(".safetensors")
        is_gguf = filename.lower().endswith(".gguf")
    else:
        filename = model_info.filename
        is_safetensors = model_info.is_safetensors
        is_gguf = model_info.is_gguf

    if device is None:
        device = torch.device("cpu")
    if is_safetensors:
        sd = safetensors.torch.load_file(filename, device=device.type)
    elif is_gguf:
        ...
        if safe_load:
            if not 'weights_only' in torch.load.__code__.co_varnames:
                ...
                safe_load = False
        if safe_load:
            pl_sd = torch.load(filename, map_location=device, weights_only=True)
        else:
            pl_sd = torch.load(filename, map_location=device, pickle_module=backend.misc.checkpoint_pickle)
