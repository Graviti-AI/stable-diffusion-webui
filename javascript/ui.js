...
function set_theme(theme) {
  ...
  if (!gradioURL.includes('__theme=')) {
    window.location.replace(
      `${window.location.origin}?${searchParam}&__theme=${theme}`
    );
  }
}
...
async function tierCheckGenerate(tabname) {
  ...
  for (let permission of permissions.generate) {
    ...
    if (permission.type === 'checkbox') {
      const target_elem = document.querySelector(
        `#${tab_elem_id} #${elem_id} input`
      );
...
async function calcuCreditTimes(
  width,
  height,
  batch_count,
  batch_size,
...
function updateGenerateBtn_txt2img(
  width = 512,
  height = 512,
  batch_count = 1,
  batch_size = 1,
  ...
  debounceCalcute['txt2img_generate'](
    width,
    height,
    batch_count,
    batch_size,
...
function updateGenerateBtn_img2img(
  width = 512,
  height = 512,
  batch_count = 1,
  batch_size = 1,
  ...
  debounceCalcute['img2img_generate'](
    width,
    height,
    batch_count,
    batch_size,
...
function redirect_to_payment_factory(boxId) {
  function redirect_to_payment(need_upgrade) {
    if (need_upgrade) {
      ...
      if (
        need_upgrade_obj.hasOwnProperty('need_upgrade') &&
        need_upgrade_obj.need_upgrade
      ) {
        const message = need_upgrade_obj.hasOwnProperty('message')
          ? need_upgrade_obj.message
        ...
        notifier.confirm(message, onOk, onCancel, {
          labels: {
            confirm: 'Upgrade Now',
            confirmOk: 'Upgrade',
          },
...
function currentImg2imgSourceResolution(w, h, scaleBy) {
  ...
  if (img) {
    const maxScale = Math.min(
      Math.floor(maxImgSizeLimit / img.naturalWidth),
      Math.floor(maxImgSizeLimit / img.naturalHeight)
    ).toFixed(2);
...
function switchWidthHeight(tabname) {
  var width = gradioApp().querySelector(
    '#' + tabname + '_width input[type=number]'
  );
  var height = gradioApp().querySelector(
...
function browseWorkspaceModels() {
  const browseModelsBtn = gradioApp().querySelector(
    '#browse_models_in_workspace'
  );
  if (
    gradioApp()
      .querySelector('div#img2img_extra_networks')
  ...
  fetchHomePageDataAndUpdateList({
    tabname: 'img2img',
    model_type: currentTab.get('img2img'),
    page: 1,
  });
}
...
function searchModel({ page_name, searchValue }) {
  const requestUrl = connectNewModelApi
    ? `/internal/favorite_models?model_type=${model_type_mapper[page_name]}&search_value=${searchValue}&page=1&page_size=${uiPageSize}`
...
function searchPublicModel({ page_name, searchValue }) {
  const requestUrl = connectNewModelApi
    ? `/internal/models?model_type=${model_type_mapper[page_name]}&search_value=${searchValue}&page=1&page_size=${uiPageSize}`
    : `/sd_extra_networks/models?page_name=${page_name}&page=1&search_value=${searchValue}&page_size=${uiPageSize}&need_refresh=false`;
  return fetchGet(requestUrl);
}
...
async function getModelFromUrl() {
  ...
  for (const [key, value] of urlList) {
    if (keyMapModelType[key]) {
      if (checkpoint) {
        notifier.alert(
          'There are multiple checkpoint in the url, we will use the first one and discard the rest'
        );
        break;
      }
      if (key === 'checkpoint') {
      ...
      const publicModelResponse = searchPublicModel({
        page_name: keyMapModelType[key],
  ...
  notifier.asyncBlock(allPromise, async (promisesRes) => {
    promisesRes.forEach(async (publicModelResponse, index) => {
      if (publicModelResponse && publicModelResponse.status === 200) {
        const { model_list, allow_negative_prompt } =
          await publicModelResponse.json();
        if (model_list && model_list.length > 0) {
          // add to personal workspace
          const res = await fetchPost({
            data: { model_id: model_list[0].id },
            url: `/internal/favorite_models`,
          });
          if (res.status === 200) {
            notifier.success(
              `Added model ${model_list[0].name} to your workspace successfully.`
            );
            ...
            notifier.alert(
              `Added model ${model_list[0].name} to your workspace Failed`
            );
          }
          ...
          notifier.warning(
            `We could not find model (${urlValues[index]}) in our library. Trying to download it from Civitai, it could take up to 5 minutes. Meanwhile, feel free to check out thousands of models already in our library.`,
            {
              labels: {
...
function getSubscribers(interval = 10, timeoutId = null) {
  const latestSubscribers = fetchGet(
    `/subscriptions/latest?interval=${interval}`
  );
      ...
      if (newSubscribers.current_tier != 'free') {
        ...
        newSubscribers.subscribers.forEach((newSubscriber) => {
          ...
          if (typeof posthog === 'object') {
            notificationElem
              .querySelector('.notification-upgrade-hyperlink')
...
async function checkSignatureCompatibility(timeoutId = null) {
  ...
  const currentTxt2imgSignature = gradioApp().querySelector(
    '#txt2img_signature textarea'
  ).value;
  const currentTxt2imgFnIndex = gradioApp().querySelector(
    '#txt2img_function_index textarea'
  ).value;
  const currentImg2imgSignature = gradioApp().querySelector(
    '#img2img_signature textarea'
  ).value;
  const currentImg2imgFnIndex = gradioApp().querySelector(
    '#img2img_function_index textarea'
  ).value;
  ...
  txt2imgSignaturePromise
    ...
    .then((txt2imgSignature) => {
      if (
        txt2imgSignature &&
        txt2imgSignature.signature &&
        txt2imgSignature.fn_index
      ) {
        if (
          (txt2imgSignature.signature != currentTxt2imgSignature ||
            txt2imgSignature.fn_index != currentTxt2imgFnIndex) &&
          !needRefresh
        ) {
          let onRefresh = () => {
            location.reload();
  ...
  img2imgSignaturePromise
    ...
    .then((img2imgSignature) => {
      if (
        img2imgSignature &&
        img2imgSignature.signature &&
        img2imgSignature.fn_index
      ) {
        if (
          (img2imgSignature.signature != currentImg2imgSignature ||
            img2imgSignature.fn_index != currentImg2imgFnIndex) &&
          !needRefresh
        ) {
          let onRefresh = () => {
            location.reload();
...
async function joinShareGroupWithId(
  share_id,
  userName = null,
  userAvatarUrl = null
) {
  ...
  if (!showed) {
    ...
    fetch('/share/group/create/html', {
      ...
      .then((data) => {
        ...
        const shareButton = doc.body.querySelector(
          'button.share-group-share-btn'
        );
        const checkStatusButton = doc.body.querySelector(
          'button.share-group-status-btn'
        );
    ...
    .then((result) => {
      if (result) {
        ...
        if (userInfo) {
          ...
          if (
            ['basic', 'plus', 'pro', 'api'].includes(result.tier.toLowerCase())
          ) {
        ...
        const boostButton = gradioApp().querySelector(
          '#one_click_boost_button'
        );
