...
from modules.processing import Processed, StableDiffusionProcessingImg2Img, process_images, build_decoded_params_from_processing
...
from modules.system_monitor import generate_function_name, monitor_call_context
...
def process_batch(p, input, output_dir, inpaint_mask_dir, args, to_scale=False, scale_by=1.0, use_png_info=False, png_info_props=None, png_info_dir=None):
    ...
    for i, image in enumerate(batch_images):
        ...
        with monitor_call_context(
                ...
                f"{generate_function_name(img2img)}.batch",
...
def img2img(request: gr.Request, id_task: str, mode: int, prompt: str, negative_prompt: str, prompt_styles, init_img, sketch, sketch_fg, init_img_with_mask, init_img_with_mask_fg, inpaint_color_sketch, inpaint_color_sketch_fg, init_img_inpaint, init_mask_inpaint, mask_blur: int, mask_alpha: float, inpainting_fill: int, n_iter: int, batch_size: int, cfg_scale: float, distilled_cfg_scale: float, image_cfg_scale: float, denoising_strength: float, selected_scale_tab: int, height: int, width: int, scale_by: float, resize_mode: int, inpaint_full_res: bool, inpaint_full_res_padding: int, inpainting_mask_invert: int, img2img_batch_input_dir: str, img2img_batch_output_dir: str, img2img_batch_inpaint_mask_dir: str, restore_faces: bool, override_settings_texts, img2img_batch_use_png_info: bool, img2img_batch_png_info_props: list, img2img_batch_png_info_dir: str, img2img_batch_source_type: str, img2img_batch_upload: list, *args):
    ...
    if mode == 0:  # img2img
        ...
        if not inpaint_color_sketch or not inpaint_color_sketch_fg:
    ...
    gallery_ids = images.build_url_to_image_ids(processed.images)
