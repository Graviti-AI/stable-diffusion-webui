...
def image_from_url_text(filedata):
    ...
    if isinstance(filedata, str):
        ...
        if filedata.startswith("data:image/") and "base64," in filedata:
            filedata = filedata.split("base64,", 1)[1]
...
def parse_generation_parameters(x: str, skip_fields: list[str] | None = None, *, request: gr.Request):
...
```
    ...
    *lines, lastline = x.strip().split("\n")
    ...
    if shared.opts.infotext_styles != "Ignore":
        found_styles, prompt_no_styles, negative_prompt_no_styles = shared.prompt_styles(request).extract_styles_from_prompt(prompt, negative_prompt)
