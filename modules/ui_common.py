...
def save_files(request: gr.Request, js_data, images, do_make_zip, index):
    ...
    with (open(logfile_path, "a", encoding="utf8", newline='') if shared.opts.save_write_log_csv else nullcontext()) as file:
        ...
        for image_index, filedata in enumerate(images, start_index):
            ...
            fullfn, txt_fullfn, _ = modules.images.save_image(image, save_to, "", seed=p.all_seeds[i], prompt=p.all_prompts[i], extension=extension, info=p.infotexts[image_index], grid=is_grid, p=p, save_to_dirs=save_to_dirs)
